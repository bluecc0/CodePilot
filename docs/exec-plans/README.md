...
| 文件 | 主题 | 状态 |
|------|------|------|
| [active/multi-character-group-chat.md](active/multi-character-group-chat.md) | **多角色卡与群聊编排**：独立实现 V1/V2/V3 JSON/PNG 角色卡、角色单聊/群组管理、持久化顺序批次、隐私遮罩与本地壁纸，以及后续自然发言与协作合同 | 🟡 单聊/群聊/个性化 code complete；真实 Provider smoke 待执行 |
